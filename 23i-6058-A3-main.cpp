...
// To my testing everything is working.
...
int hashing::hashfunction(string key) {
    ...
    for (int i = 0; i < key.length(); i++) {
        index = (index * base + key[i]) % size; 
    }
    return index;
}

void hashing::insert() {
    ifstream rep_file("Assig03_wordlist.txt"); // !!!!File Name might be different!!!!
...
bool hashing::search() {
    ifstream input_file("input.txt"); //!!!!! FILE NAME MIGHT BE DIFFERENT!!!!!!
    ofstream output_file("input.txt", ios::app); 
...
void hashing::loadfactor() {
    float loadfactor = static_cast<float>(Num_element) / static_cast<float>(size); // had to static cast otherwise it wasn't working properly
    cout << "--------------------" << endl;
